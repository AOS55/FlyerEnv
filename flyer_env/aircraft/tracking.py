...
class TrackPoints:
    ...
    def _unit_dir_vector(start_point, end_point):
        ...
        try:
            unit_vector_n = direction_vector[0] / np.sqrt(np.power(direction_vector[0], 2)
                            + np.power(direction_vector[1], 2))
        except ZeroDivisionError:
            unit_vector_n = 0
        try:
            unit_vector_e = direction_vector[1] / np.sqrt(np.power(direction_vector[0], 2)
                            + np.power(direction_vector[1], 2))
        except ZeroDivisionError:
            unit_vector_e = 0
        ...
        if self.goal_state < len(self.goal_set) - 2:
            ...
            if self.control_state == 1:
                ...
                if h_val < 0:
                    self.control_state = 0
                    self.goal_state += 1
                distance_from_center = np.sqrt(np.power(pos[0] - center_point[0], 2) +
