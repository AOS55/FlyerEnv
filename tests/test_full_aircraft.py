import pytest
import numpy as np
from tests.common.base_test import BaseEnvironmentTest, EnvironmentConfigs, ObservationValidator, StateValidator

class TestFullAircraftEnvironment(BaseEnvironmentTest):
    def get_config(self):
    ...
    def test_control_surface_responses(self, control_input):
        ...
        if control_input[0] != 0:  # Elevator
            assert obs['q'] != initial_state['q'], "No pitch rate response to elevator"
            
        if control_input[1] != 0:  # Aileron
            assert obs['p'] != initial_state['p'], "No roll rate response to aileron"
            
        if control_input[2] != 0:  # Throttle
            assert obs['u'] != initial_state['u'], "No speed response to throttle"
            
        if control_input[3] != 0:  # Rudder
            assert obs['r'] != initial_state['r'], "No yaw rate response to rudder"
    ...
    def test_basic_maneuvers(self, maneuver):
        ...
        if name == "level_turn":
            assert obs['yaw'] != initial_state['yaw'], "No heading change in turn"
            # Check for roughly constant altitude
            assert abs(obs['z'] - initial_state['z']) < 10.0, "Significant altitude change in level turn"
            
        elif name == "climb":
            assert obs['z'] < initial_state['z'], "No altitude gain in climb"  # Remember NED coordinates
            
        elif name == "descent":
            assert obs['z'] > initial_state['z'], "No altitude loss in descent"  # Remember NED coordinates
    ...
    def test_invalid_inputs(self):
        """Test handling of invalid control inputs"""
        self.env.reset()
        
        invalid_inputs = [
            np.array([np.inf, 0, 0, 0]),    # Infinite value
            np.array([0, np.nan, 0, 0]),    # NaN value
            np.array([1.5, 1.5, 1.5, 1.5]), # Out of bounds
            np.array([0, 0, 0]),            # Wrong size
        ]
        
        for invalid_action in invalid_inputs:
